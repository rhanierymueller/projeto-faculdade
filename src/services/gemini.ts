        ...
        try {
          ...
          if (response.ok) {
            ...
            if (modelIndex > 0 || attempt > 0) {
              console.log(
                `✓ Sucesso com modelo fallback: ${currentModel} (tentativa ${
                  attempt + 1
                })`
              );
            }
          ...
          if (response.status === 404) {
            console.warn(
              `Modelo ${currentModel} não encontrado. Tentando próximo modelo...`
            );
            break;
          }
          ...
          if (response.status === 503) {
            ...
            if (isLastAttempt && !isLastModel) {
              console.warn(
                `Modelo ${currentModel} sobrecarregado. Tentando próximo modelo...`
              );
              break;
            } else if (!isLastAttempt) {
              const delay = INITIAL_RETRY_DELAY * Math.pow(2, attempt);
              console.warn(
                `Modelo ${currentModel} sobrecarregado. Retentando em ${delay}ms...`
              );
              await sleep(delay);
              continue;
            }
          ...
          if (
            ...
            console.warn(
              `Erro com modelo ${currentModel}. Tentando próximo modelo...`
            );
            break;
          }
